...
public abstract class AbstractLoanRepaymentScheduleTransactionProcessor implements LoanRepaymentScheduleTransactionProcessor {
    ...
    private void handleDisbursementChargeBalanceCredit(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
        ...
        for (final LoanRepaymentScheduleInstallment installment : installments) {
            ...
            if (installmentPrincipalPortion.plus(installmentInterestPortion).plus(installmentPenaltyChargesPortion).isGreaterThanZero()) {
                ...
                chargeBalanceReplayLedger.recordInstallmentCredit(installment, installmentPrincipalPortion, installmentInterestPortion,
                        installmentPenaltyChargesPortion);
            }
        }
    ...
    private Money handleTransactionAndCharges(final LoanTransaction loanTransaction, final MonetaryCurrency currency,
        ...
        if (loanTransaction.isNotWaiver() && !loanTransaction.isAccrual()) {
            ...
            if (feeCharges.isGreaterThanZero()) {
                updateChargesPaidAmountBy(loanTransaction, feeCharges, loanFees, installmentNumber, false);
            }

            if (penaltyCharges.isGreaterThanZero()) {
                updateChargesPaidAmountBy(loanTransaction, penaltyCharges, loanPenalties, installmentNumber, true);
            }
        }
    ...
    private void updateChargesPaidAmountBy(final LoanTransaction loanTransaction, final Money feeCharges, final Set<LoanCharge> charges,
            final Integer installmentNumber, final boolean isPenaltyPortion) {
        ...
        if (amountRemaining.isGreaterThanZero()) {
            loanTransaction.reduceExcessChargePortionToOverpayment(feeCharges.getCurrency(), amountRemaining, isPenaltyPortion);
        }
    ...
    private LoanCharge findEarliestUnpaidChargeFromUnOrderedSet(final Set<LoanCharge> charges, final MonetaryCurrency currency) {
        ...
        for (final LoanCharge loanCharge : charges) {
            if (loanCharge.isActive() && loanCharge.getAmountOutstanding(currency).isGreaterThanZero()
        ...
        for (final LoanRepaymentScheduleInstallment currentInstallment : installments) {
            if (currentInstallment.isNotFullyPaidOff()) {
                if (principalPortion.isGreaterThanZero()) {
                    ...
                    if (principalPortion.isZero() && interestPortion.isZero() && feeChargesPortion.isZero()
                            && penaltyChargesPortion.isZero()) {
                        break;
                    }
                }
                ...
                if (penaltyChargesPortion.isGreaterThanZero()) {
                    Money penaltyChargesWriteOff = currentInstallment.writeOffPartialPenaltyCharges(transactionDate, currency,
