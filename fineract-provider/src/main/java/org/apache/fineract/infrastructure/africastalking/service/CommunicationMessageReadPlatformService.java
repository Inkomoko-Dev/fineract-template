...
public class CommunicationMessageReadPlatformService {
    ...
    public List<CommunicationMessageData> retrieveWhatsAppMessages() {
        return communicationMessageRepository.findByChannelOrderByCreatedDateDesc(CommunicationChannel.WHATSAPP).stream()
                .map(CommunicationMessageData::fromEntity).collect(Collectors.toList());
    }
}
