...
public class ProvisioningEntriesReadPlatformServiceImpl implements ProvisioningEntriesReadPlatformService {
    ...
    private static final class LoanProductProvisioningEntryMapper implements RowMapper<LoanProductProvisioningEntryData> {
        ...
        private LoanProductProvisioningEntryMapper(DatabaseSpecificSQLGenerator sqlGenerator) {
            ...
            sqlQuery = new StringBuilder(new StringBuilder()
                    ...
                    .append("(IFNULL(loan.principal_disbursed_derived, 0) - IFNULL(paymentTbl.princ_paid, 0)) AS principal_outstanding, ")

                    // Accrued Interest up to cut-off from repayment schedule
                    .append("(IFNULL(scheduleInterestTbl.interest_due_to_cutoff, 0) - IFNULL(paymentTbl.int_paid, 0)) AS accrued_interest_to_cutoff, ")

                    // Total Exposure = Principal Outstanding + Accrued Interest
                    .append("((IFNULL(loan.principal_disbursed_derived, 0) - IFNULL(paymentTbl.princ_paid, 0)) ")
                    .append("+ (IFNULL(scheduleInterestTbl.interest_due_to_cutoff, 0) - IFNULL(paymentTbl.int_paid, 0))) AS total_exposure, ")

                    // Provision Amount
                    .append("(((IFNULL(loan.principal_disbursed_derived, 0) - IFNULL(paymentTbl.princ_paid, 0)) ")
                    .append("+ (IFNULL(scheduleInterestTbl.interest_due_to_cutoff, 0) - IFNULL(paymentTbl.int_paid, 0))) ")
                    .append("* IFNULL(pcd.provision_percentage, 0) / 100) AS provision_amount, ")
                    ...
                    .append("AND ((lrs.completed_derived = 0 AND lrs.obligations_met_on_date IS NULL) OR lrs.obligations_met_on_date > DATE(?)) ")
                    ...
                    .append("LEFT JOIN (SELECT rs.loan_id, SUM(IFNULL(rs.interest_amount, 0)) AS interest_due_to_cutoff ")
                    .append("FROM m_loan_repayment_schedule rs ")
                    .append("WHERE rs.duedate <= DATE(?) ")
                    .append("GROUP BY rs.loan_id) AS scheduleInterestTbl ON scheduleInterestTbl.loan_id = loan.id ")
                    ...
                    .append("LEFT JOIN (SELECT t.loan_id, SUM(IFNULL(t.amount, 0)) AS Actual_paid, ")
                    .append("SUM(IFNULL(t.principal_portion_derived, 0)) AS princ_paid, ")
                    .append("SUM(IFNULL(t.interest_portion_derived, 0)) AS int_paid, ")
                    .append("SUM(IFNULL(t.penalty_charges_portion_derived, 0)) AS charge_paid, ")
                    .append("SUM(IFNULL(t.fee_charges_portion_derived, 0)) AS fee_paid ")
                    .append("FROM m_loan_transaction t ")
                    .append("WHERE (t.transaction_type_enum = 2 OR t.transaction_type_enum = 5) ")
                    .append("AND t.is_reversed = 0 AND (DATE(t.transaction_date) <= DATE(?)) ")
                    ...
                    .append("AND ((sch1.completed_derived = 0 AND sch1.obligations_met_on_date IS NULL) ")
                    .append("OR sch1.obligations_met_on_date > DATE(?))) ")
                    .append("ORDER BY numberofdaysoverdue")
                    .toString());
